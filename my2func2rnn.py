...
def sparseLSTMCell(input, hidden, w_ih, w_hh, b_ih=None, b_hh=None, lambda_sh = None):
    ...
    #     temp = D_sh.matmul(F.tanh(hy[i][:] + mu) + F.tanh(hy[i][:] - mu))
    #     temp = temp.data.numpy()
    #     hy_spa.append(temp)
    # hy_spa = Variable(torch.FloatTensor(np.array(hy_spa)))
    lambda_sh =  lambda_sh.data[0]
    hy_spa = F.hardshrink(hy,lambd=lambda_sh)
    return hy_spa, cy
    #return hy, cy
